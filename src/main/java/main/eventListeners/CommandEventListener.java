...
public class CommandEventListener extends ListenerAdapter {
    ...
    private void privateCommands(MessageReceivedEvent event, String messageContent) {
        if  (event.isFromType(ChannelType.PRIVATE)) {
            CommandList.getPrivateCommands().stream().filter(c -> c.keyMatches(messageContent.toLowerCase(Locale.ROOT))).forEach(c -> c.start(event));
        }
    }
    ...
    private void publicCommands(MessageReceivedEvent event, String messageContent) {
        if  (!event.isFromType(ChannelType.PRIVATE) && isGuildChannel(event) && isTextChannel(event) ) {
            CommandList.getCommands().stream().filter(c -> c.keyMatches(messageContent.toLowerCase(Locale.ROOT))).forEach(c -> c.start(event));
        }
    }

    private boolean isTextChannel(MessageReceivedEvent event) {
        return ServerGame.getGames().get(event.getGuild().getIdLong()).getConfig().getBot_channel().equals(event.getTextChannel().getIdLong());
    }
    ...
    private void botResponse (MessageReceivedEvent event) {
        if( isBotMessage(event)){
            addReaction(ServerGame.getGames().get(event.getGuild().getIdLong()).getConfig().getYeet_emote(),event);
            addReaction(ServerGame.getGames().get(event.getGuild().getIdLong()).getConfig().getYeetnt_emote(),event);
        }
    }
    ...
    public void onMessageReceived(@NotNull MessageReceivedEvent event) {
        ...
        if(event.isFromGuild() && ServerGame.getGames().get(event.getGuild().getIdLong()) == null ) {
            ServerGame.getGames().put(event.getGuild().getIdLong(),new SHGame(new Gamestate(), new PlayerList(), null));
        }
